...
class SociosMember(models.Model, Suscriber):
    ...
    mirror_dependencies = [
        "infocoop_socios", "infocoop_tablas", "infocoop_suscrip", "infocoop_ingresos", ]
    ...
    def prepare_row_fields(self, row):
        ...
        if row.cuit:
            doc_type = self.env["res.partner.id_category"].search(
                (["active", "=", True], ["code", "=", "CUIT"]), limit=1)
            doc_number = re.sub('[-]', '', row.cuit)

        localidad = self.env["infocoop_tablas"].search(
            (["tema", "=", "T"], ["subtema", "=", "L"], ["codigo", "=", row.codloca]), limit=1)
        if localidad:
        ...
        minutes_id = None
        gender = None
        ingreso = self.env["infocoop_ingresos"].search(
            [("socio", "=", row.nrosoc), ], limit=1)
        if ingreso:
            minutes_id = self.env["minutes"].search(
                (["number", "=", ingreso.acta],), limit=1).id
            if not minutes_id:
                minutes_id = self.env["minutes"].create(
                    {"number": ingreso.acta, "date": ingreso.fec_acta}).id

            if not doc_number:
                # get doc data from ingresos
                doc_type, doc_number = doc_number_normalize(
                    ingreso.tipo_doc, ingreso.nro_doc)

                ids = self.env["res.partner.id_category"].search(
        ...
        if doc_type:
            try:
                ...
                data["comment"] = "Identity validation fail: %s %s " % (
                    doc_type, doc_number)
                doc_type = None
        ...
        responsability_id = self.env["afip.responsability.type"].search(
            (["code", "=", code], ["active", "=", 1]), limit=1).id
        if code == "1" or code == "4":  # RI or Exento
            data["is_company"] = True

        susc = self.env["infocoop_suscrip"].search(
            [("nrosoc", "=", row.nrosoc), ], limit=1)
        if susc:
    ...
    def get_slave_from_row(self, row):
        socio = self.env["infocoop_ingresos"].search(
            [("socio", "=", row.nrosoc), ], limit=1).socio
        if socio > 0:
