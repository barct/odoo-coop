...
class Suscriber():
    ...
    Is responsible for maintaining sync data between a master record (infocoop
    mirror table row)
    and a slave record (business model)
    '''
    hashcode = fields.Char(length=10)
    _sql_constraints = [
        ('unique_keys', 'unique(master_id,slave_id)', 'must be unique!'), ]
    ...
    def sync_to_odoo(self):
        ...
        deps = self.env["infocoop.mirror_tables"].search(
            [("name", "in", self.mirror_dependencies), ])
        if deps:
            dep_str = ""
            for d in deps:
                if d.out_of_date:
    ...
    def sync_row(self, row):
        '''
        Check if slave record should be updated, deleted or created based
        on a master row and does it
        '''
        s_ids = self.env[self._name].search(
            (["master_id", "=", row.id],), limit=1)
        if s_ids:
            if row.sync_delete_mark:
                if s_ids.slave_id is not None:
    ...
    def get_slave_from_row(self, row):
        '''
        This method must be used to obtain a slave record
        based on a mirror dbf row
        '''
        raise Exception("This method must be overwrited in implementation")

    def create_from_infocoop(self, row):
        '''When table mirror has a new record do this:
        Create a new record  master-slave
        '''
        new = self.env[self.slave_id._name].create(
            (self.prepare_row_fields(row)))
        obj = self.create(
            {"slave_id": new.id,
             "master_id": row.id,
             "hashcode": row.hashcode})
    ...
    def update_from_infocoop(self, row):
        '''When table mirror has a change in a record do this:
        Send to update process'''
    ...
    def delete_from_infocoop(self, row):
        '''When table mirror has deleted record, do this:
        Delete slave record'''
