...
class CtaCteInvoice(models.Model, Suscriber):
	...
	def prepare_row_fields(self, row):
		...
		journal_ids = self.env["account.journal"].search([("point_of_sale","=",int(row.sucursal),)], limit=1)
		journal_id = None
		...
		if not journal_id:
			raise Warning(_("Journal for voucher: %s, and point of sale: %s not found!" % (row.letra, row.sucursal)))
		...
		if row.comproban == "F":
			if row.letra == "A":
				document_type_id = env_ref('l10n_ar_account.dc_a_f')
			else:
				document_type_id = env_ref('l10n_ar_account.dc_b_f')
		elif row.comproban == "D":
			if row.letra == "A":
				document_type_id = env_ref('l10n_ar_account.dc_a_nd')
			else:
				document_type_id = env_ref('l10n_ar_account.dc_b_nd')
		elif row.comproban == "C":
			if row.letra == "A":
				document_type_id = env_ref('l10n_ar_account.dc_a_nc')
			else:
				document_type_id = env_ref('l10n_ar_account.dc_b_nc')
	...
	def get_slave_form_row(self, row):		
		return self.env["account.payment"].search([("document_number","=",row.num_fact[-12:-8].replace(" ","0") + "-" + row.num_fact[-8:]),], limit=1)	
